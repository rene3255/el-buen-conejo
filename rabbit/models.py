...
class ActiveRabbitManager(models.Manager):
    def get_queryset(self):
        return super(ActiveRabbitManager, self).get_queryset().filter(is_active=True)


class MaleRabbitManager(models.Manager):
    def get_queryset(self):
        return (
            super(MaleRabbitManager, self)
            .get_queryset()
            .filter(male="M", is_active=True)
        )
...
class DoeRabbitManager(models.Manager):
    def get_queryset(self):
        return (
            super(DoeRabbitManager, self)
            .get_queryset()
            .filter(rabbit_status__status="Reproductora", is_active=True, is_doe=False)
        )
...
class BuckRabbitManager(models.Manager):
    def get_queryset(self):
        return (
            super(BuckRabbitManager, self)
            .get_queryset()
            .filter(rabbit_status__status="Semental", is_active=True, is_buck=False)
        )
...
class Rabbit(models.Model):
    ...
    sex = models.CharField(max_length=1, choices=RABBIT_SEX, default=RABBIT_SEX[0][1])
    birth_date = models.DateField(null=True, blank=True)
    rabbit_photo = models.ImageField(
        "Foto del conejo",
        upload_to="media/rabbits/",
        default="rabbit_avatar.png",
        null=True,
        blank=True,
    )
    ...
    def rabbit_tag(self):
        ...
        return (
            rabbit_tag.build_rabbit_tag()
        )  # f"{str_tag[:4]}-{self.formatted_id_number(self.id)}"
    ...
    def __str__(self):
        ...
        result = self.rabbit_tag + " " + str(self.rabbit_status) + "\n" + complete_sex
        return result


class RabbitTagTemplate:

    def __init__(self, farm_name, model_id, model_id_length=3, character_type="0"):
    ...
    def model_id_template(self):
        ...
        pattern = model_id_plus_pattern[
            len(str(self.model_id)) : len(model_id_plus_pattern)
        ]
        return pattern
